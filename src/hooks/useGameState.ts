...
export const useGameState = () => {
  ...
  const completeMatch = useCallback(() => {
    ...
    const playerTeamScore = isPlayerHome ? finalHomeScore : finalAwayScore;
    const opponentTeamScore = isPlayerHome ? finalAwayScore : finalHomeScore;
    ...
    const result = playerTeamScore > opponentTeamScore ? 'win' : playerTeamScore < opponentTeamScore ? 'loss' : 'draw';
    ...
    if (gameState.player.position === 'Forward' && eventSuccesses > 0) {
      updatedPlayer.goals += Math.min(eventSuccesses, playerTeamScore);
    } else if (gameState.player.position === 'Midfielder' && eventSuccesses > 0) {
      updatedPlayer.assists += Math.min(Math.floor(eventSuccesses / 2), playerTeamScore);
    } else if (gameState.player.position === 'Goalkeeper' && playerTeamScore === 0) {
      updatedPlayer.cleanSheets += 1;
    } else if (gameState.player.position === 'Defender' && opponentTeamScore === 0) {
      updatedPlayer.cleanSheets += 1;
    }
    ...
    const updatedTable = gameState.season.leagueTable.map(record => {
      if (record.team === gameState.playerTeam) {
        return {
          ...
          goalsFor: record.goalsFor + playerTeamScore,
          goalsAgainst: record.goalsAgainst + opponentTeamScore,
          goalDifference: record.goalDifference + (playerTeamScore - opponentTeamScore),
